...
segments_dtype = np.dtype([("eventID", "u4"), ("segment_id", "u4"), ("z_end", "f4"),
                           ...
                           ("dEdx", "f4"), ("dEdx_secondary", "f4"), 
                           ("dEdx_ionizing", "f4"), ("dEdx_nonionizing", "f4"), 
                           ("dE", "f4"), ("dE_secondary", "f4"), ("dE_ionizing", "f4"), 
                           ("dE_nonionizing", "f4"), ("t", "f4"),
                           ("y", "f4"), ("x", "f4"), ("z", "f4"),
    ...
    for jentry in tqdm(range(entries)):
        ...
        for containerName, hitSegments in event.SegmentDetectors:
            ...
            for iHit, hitSegment in enumerate(hitSegments):
                ...
                segment[iHit]["dE_ionizing"] = 2*hitSegment.GetEnergyDeposit() - hitSegment.GetSecondaryDeposit()
                segment[iHit]["dE_nonionizing"] = hitSegment.GetSecondaryDeposit() - hitSegment.GetEnergyDeposit()
                # import particle
                # print ("PID:", segment[iHit]["pdgId"])
                # try:
                #     print ("PID:", particle.Particle.from_pdgid(segment[iHit]["pdgId"]))
                # except particle.InvalidParticle or particle.ParticleNotFound:
                #     continue
                # print ("secondary/primary:", segment[iHit]["dE_nonionizing"]/segment[iHit]["dE"])
                # print ("secondary/primary:", hitSegment.GetSecondaryDeposit()/hitSegment.GetEnergyDeposit())
                # print ("ionizing:", segment[iHit]["dE_ionizing"])
                # print ("nonionizing:", segment[iHit]["dE_nonionizing"])
                ...
                segment[iHit]["dEdx_secondary"] = segment[iHit]["dE_secondary"] / dx if dx > 0 else 0
                segment[iHit]["dEdx_ionizing"] = segment[iHit]["dE_ionizing"] / dx if dx > 0 else 0
                segment[iHit]["dEdx_nonionizing"] = segment[iHit]["dE_nonionizing"] / dx if dx > 0 else 0
